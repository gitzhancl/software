...
define([
    ...
    function (app, services, BizConfig, MenuConfig,  bonuscmsOfflineEdit) {

        bonuscmsOfflineEdit.injectTo(app);

        app.controller('OfflineCreateCtrl', ['$scope', '$location', '$route', '$filter',
            'bonuscmsOfflineEditService',
            'validService',
            'authorityService',

            function ($scope, $location, $route, $filter, bonuscmsOfflineEditService,
                      validService) {
                ...
                $scope.btns = {
                    "save": function () {
                        ...
                        $.when(bonuscmsOfflineEditService.sendRequest({"action":'save',"json": angular.toJson($scope.offline)}))
                            .done(function (result) {
                                alert(result.debugMessage+"！  即将返回列表...")
                                if(result.code==1){
